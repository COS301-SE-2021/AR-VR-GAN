...
if __name__ == "__main__":
    model = torch.load("savedModels/Epochs-100.pt")
    with torch.no_grad():
        # Returns a 1x20 array with random values from 0-1
        sample = torch.randn(1,20).to(device)
        print(f"Shape Sample: {sample.size()}")
        sample = model.decode(sample).cpu()
        transform = transforms.Compose([ transforms.ToPILImage(), transforms.Resize(size=100), transforms.ToTensor() ]) 
        # x = [transform(x_) for x_ in sample]
        # x = transform(sample)  
        # torchvision.utils.save_image(x, 'test.png')
        save_image(sample.view(1, 1, 28, 28),'testing.png')
